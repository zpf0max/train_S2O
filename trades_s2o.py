...
def criterion(model,model_old,fisher2):
...
def trades_loss(model,
    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            x_adv = x_adv.detach() + step_size * torch.sign(grad[0].detach())
    ...
    loss_robust = (1.0 / batch_size) * criterion_kl(F.log_softmax(model(x_adv), dim=1),
                                                    F.softmax(model(x_natural), dim=1))
    loss_reg=criterion(model,model_old,fisher_old)
   
    loss = loss_natural*0.65 + 0.4*0.5*(loss_yy+loss_yy_adv) + beta * loss_robust+loss_reg
    #print("loss_reg",5000*loss_reg)
    #print("loss",loss)
    return loss
